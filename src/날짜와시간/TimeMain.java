...
public class TimeMain {
    public static void main(String[] args) {
        LocalDate date = LocalDate.now(); // 운영체제로 부터 현재 날짜를 가져옴
        LocalTime time = LocalTime.now(); // 현재 시간을 가져 옴
        LocalDateTime dateTime = LocalDateTime.now(); // 날짜와 시간을 가져옴.나노초?
        ZonedDateTime zoned = ZonedDateTime.now(); // GMT 까지 표기.

        System.out.println(date);
        System.out.println(time);
        System.out.println(dateTime);
        System.out.println(zoned);
        // HH < 24시간 형식, hh < 12시간 형식
        // AM, PM 의 표시 형식은 .withLocale(Locale.국가);

        // 2025년 10월 17일 오전 9시39분
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy년 MM월 dd일 a hh시 mm분 ss초");
        System.out.println(dateTime.format(formatter));

        // 2025/10/17 AM 09:39:38
        DateTimeFormatter formatter1 = DateTimeFormatter.ofPattern("yyyy/MM/dd a hh:mm:ss").withLocale(Locale.US);
        System.out.println(dateTime.format(formatter1));

        // 2025-10-17 23:25:25 (24시간제로 표시)
        DateTimeFormatter formatter2 = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        System.out.println(dateTime.format(formatter2));

        // 25년 10월 17일 (금) => 요일은 E
        DateTimeFormatter formatter3 = DateTimeFormatter.ofPattern("yy년 MM월 dd일 (E)");
